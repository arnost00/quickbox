	...
	: Super(parent)
{
	...
	m_btStage = new QPushButton(this);
	m_btStage->setToolTip(tr("Set current stage"));
	connect(m_btStage, &QPushButton::clicked, this, &AppStatusBar::stageClicked);
	addPermanentWidget(m_btStage);
...
{
	m_stageNo = stage_no;
	m_btStage->setText(tr("E%1").arg(m_stageNo));
}
