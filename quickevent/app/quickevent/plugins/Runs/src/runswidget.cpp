...
void RunsWidget::onDrawClicked()
{
	...
	try {
		...
		for(int class_id : class_ids) {
			int handicap_length_ms = getPlugin<EventPlugin>()->appDbConfig()->handicapLength() * 60 * 1000;
			...
			if(draw_method == DrawMethod::RandomNumber) {
				...
				int stage_count = getPlugin<EventPlugin>()->appDbConfig()->stageCount();
...
void RunsWidget::export_results_stage_csv()
{
	...
	bool is_iof_race = getPlugin<EventPlugin>()->appDbConfig()->isIofRace();
